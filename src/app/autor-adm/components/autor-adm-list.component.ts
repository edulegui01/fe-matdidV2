...
@Component({
  selector: 'app-autor-adm-list',
  templateUrl: '../templates/autor-adm-list.component.html',
  styleUrls: ['../styles/autor-adm-list.component.scss']
})
export class AutorAdmListComponent  implements OnInit {
  ...
  constructor(private autorAdmService:AutorAdmService, private paginator: MatPaginatorIntl, private fb:FormBuilder, private routerInstance: Router, private dialogInstance: MatDialog) {
  }
  ...
  initDataSource(){
    this.autorAdmService.getAutores().subscribe( (autoresData:any) => this.dataSource = autoresData)
  }
  ...
  onPaginateChange(event:PageEvent){
    ...
    this.autorAdmService.getAutores(page,size,nombre).subscribe((autorData:any) => this.dataSource = autorData);
  }
  ...
  doFilter(){
    ...
    this.autorAdmService.getAutores('0','10',nombre).subscribe((autorData:any) => this.dataSource = autorData);
  }
    ...
    this.dialogInstance.open(CustomDialogComponent, {
                  ...
                  if (accept) {
                    ...
                    this.paginatorf.pageIndex = 0;
                      ...
                      this.autorAdmService.deleteLocalidad(element.id).subscribe(resp => {
                        this.paginatorf.pageIndex = 0;
                        this.autorAdmService.getAutores().subscribe( (autorData:any) => this.dataSource = autorData)
                      });
                  }
              });
  }
  ...
  OnClickEditCliente(element:any){
    ...
    this.autorAdmService.editForm=true;
    console.log(element);
    this.routerInstance.navigate(['autor-adm/editar-autor-adm'],extraParams);
