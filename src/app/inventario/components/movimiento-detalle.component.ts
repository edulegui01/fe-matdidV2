...
@Component({
  ...
  templateUrl: '../templates/movimiento-detalle.component.html',
  styleUrls: ['../styles/movimiento-detalle.component.scss']
})
export class MovimientoDetalleComponent implements OnInit {
  ...
  constructor(public inventarioService:InventarioService, private formBuilder:FormBuilder, router: Router, 
    private dialogInstance: MatDialog, private datePipe: DatePipe, private  snackbarInstance: MatSnackBar, public dialogRef: MatDialogRef<MovimientoDetalleComponent>,
    ...
    const detalleMovimientos = this.data.element.detalleMovimientos;

    detalleMovimientos.forEach((item:any) => {
        let productoToPush:FormGroup; 
        productoToPush = this.createFormGroupProducts(item);
