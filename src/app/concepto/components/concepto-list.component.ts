...
@Component({
  ...
  templateUrl: '../templates/concepto-list.component.html',
  styleUrls: ['../styles/concepto-list.component.scss']
})
export class ConceptoListComponent  implements OnInit {
  ...
  constructor(private conceptoService:ConceptoService, private paginator: MatPaginatorIntl, private fb:FormBuilder, private routerInstance: Router, private dialogInstance: MatDialog) {
  }
  ...
  initDataSource(){
    this.conceptoService.getConcepto().subscribe( (localidadData:any) => this.dataSource = localidadData)
  }
  ...
  onPaginateChange(event:PageEvent){
    ...
    this.conceptoService.getConcepto(page,size,nombre).subscribe((localidadData:any) => this.dataSource = localidadData);
  }
  ...
  doFilter(){
    ...
    this.conceptoService.getConcepto('0','10',nombre).subscribe((localidadData:any) => this.dataSource = localidadData);
  }
    ...
    this.dialogInstance.open(CustomDialogComponent, {
                  ...
                  if (accept) {
                    ...
                    this.paginatorf.pageIndex = 0;
                      ...
                      this.conceptoService.deleteConcepto(element.idConcepto).subscribe(resp => {
                        this.paginatorf.pageIndex = 0;
                        this.conceptoService.getConcepto().subscribe( (localidadData:any) => this.dataSource = localidadData)
                      });
                  }
  ...
  OnClickEditCliente(element:any){
    ...
    this.conceptoService.editForm=true;
    console.log(element);
    this.routerInstance.navigate(['concepto/editar-concepto'],extraParams);
  ...
  displayedColumns: string[] = ['nombre', 'esIngreso','options'];
